			  ...
			  <div class="col-sm-6 col-md-3 moviebox" id="movie-${item.movie_no}">
			    <div class="thumbnail">
			      ...
			      <div class="caption">
					...
					<button type="button" onclick="loadMovieViewBook(${item.movie_no})" class="btnDetail btnDetail-${item.movie_no} btn btn-primary btn-block" >
					예매하기
					</button>
					</p>
			      </div>
...
function loadMovieViewBook(movie_no){
	//alert(movie_no);
	$.ajax({
		type:"GET",
		url:"movie_view_book?movie_no="+movie_no,
		success:function(data){
			$(".modal-body").html('');
			$(".modal-body").html(data);
			$("#movieModal").modal();
